#ifndef ATLASSIMINTERFACEWRAPPER_H
#define ATLASSIMINTERFACEWRAPPER_H
...
class AtlasSimInterfaceWrapper;
...
private:
    ...
    static AtlasControlInput input;
    static AtlasControlOutput output;
    static AtlasRobotState robot_state;
public:
    ...
    AtlasErrorCode process_control_input();
    ...
    //!    \li   ERROR_NO_SUCH_BEHAVIOR
    //!
    AtlasErrorCode set_desired_behavior(const char *behavior);
    ...
    //!
    AtlasErrorCode get_desired_behavior(Atlas::AtlasBehavior &behavior);
    AtlasErrorCode get_desired_behavior(char *behavior);
    ...
    //!    \li   NO_ERRORS
    //!
    AtlasErrorCode get_current_behavior(Atlas::AtlasBehavior &behavior);
    AtlasErrorCode get_current_behavior(char *behavior);
    ...
    //!    \li   ERROR_INVALID_INDEX
    //!
    AtlasErrorCode get_behavior_at_index(int index, Atlas::AtlasBehavior  &behavior);
    ...
    //!    \li   ERROR_NO_SUCH_BEHAVIOR
    //!
    AtlasErrorCode get_behavior_joint_weights(const Atlas::AtlasBehavior  &behavior,
    ...
    //!
    const char *get_error_code_text(AtlasErrorCode ec);

    AtlasSimInterfaceWrapper();
    ~AtlasSimInterfaceWrapper();
    AtlasSimInterface* atlasSimInterface;
    static const std::string behaviors[];

    const char* get_behavior_string(Atlas::AtlasBehavior behavior);
    AtlasControlInput* getInput();

    AtlasControlOutput* getOutput();

    AtlasRobotState* getRobotState();
...
#endif // ATLASSIMINTERFACEWRAPPER_H
